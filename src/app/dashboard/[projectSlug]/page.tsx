...
async function DashboardStats({
  ...
  const activeAgents = agentStatuses.filter(
    (a) => a.status.status !== "offline"
  ).length;
  const busyAgents = agentStatuses.filter(
    (a) => a.status.status === "busy"
  ).length;
...
export default async function ProjectOverviewPage({
  params,
}: ProjectOverviewProps): Promise<ReactNode> {
  ...
  return (
    <div>
      ...
      <Suspense fallback={<StatsLoading />}>
        <DashboardStats projectSlug={projectSlug} />
      </Suspense>
    </div>
  );
}
